...
describeWithFlags('wasm init', BROWSER_ENVS, () => {
    ...
    it('backend init works when the wasm paths overrides map is valid and ' +
        'using platform fetch', async () => {
        const usePlatformFetch = true;
        const validPrefix = '/base/wasm-out/';
        setWasmPaths({
            'tfjs-backend-wasm.wasm': '/base/wasm-out/tfjs-backend-wasm.wasm',
            'tfjs-backend-wasm-simd.wasm': '/base/wasm-out/tfjs-backend-wasm.wasm',
            'tfjs-backend-wasm-threaded-simd.wasm': '/base/wasm-out/tfjs-backend-wasm.wasm'
        }, usePlatformFetch);
        let wasmPath;
        const realFetch = util.fetch;
        spyOn(util, 'fetch').and.callFake((path) => {
